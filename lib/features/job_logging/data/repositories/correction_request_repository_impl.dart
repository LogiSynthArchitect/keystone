...
class CorrectionRequestRepositoryImpl implements CorrectionRequestRepository {
  ...
  Future<CorrectionRequestEntity> createRequest(CorrectionRequestEntity request) async {
    final model = await _supabase.from(SupabaseConstants.correctionRequestsTable).insert({
  ...
  Future<List<CorrectionRequestEntity>> getMyRequests() async {
    ...
    final data = await _supabase
        .from(SupabaseConstants.correctionRequestsTable)
        .select()
        .eq('user_id', uid)
        .order('created_at', ascending: false);

    return data.map((m) => CorrectionRequestModel.fromJson(m).toEntity()).toList();
  }
  ...
  Future<void> approveRequest(String requestId, String jobId, Map<String, dynamic> updates) async {
    // 1. Update the job
    await _supabase.from(SupabaseConstants.jobsTable).update(updates).eq('id', jobId);

    // 2. Mark request as approved
    await _supabase.from(SupabaseConstants.correctionRequestsTable).update({
      'status': 'approved',
